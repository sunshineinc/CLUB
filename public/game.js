...
function onCanvasClick(event) {
    ...
    const intersects = gameState.raycaster.intersectObjects(scene.children);
...
function createGround() {
    ...
    ground.userData = { isGround: true }; // Marcar como chão para detecção de clique
    ground.name = 'ground'; // Adicionar nome para debug
    scene.add(ground);
...
function animate() {
    ...
    if (gameState.penguin.model) {
        if (gameState.penguin.isMoving && gameState.penguin.targetPosition) {
            ...
            // Mover pinguim
            const moveDistance = gameState.moveSpeed * 60 * delta; // Ajustar velocidade baseada no delta time
...
function initMultiplayer() {
    ...
    gameState.socket.onopen = () => {
        ...
        // Enviar informações iniciais do jogador
        gameState.socket.send(JSON.stringify({
            type: 'init',
            playerName: gameState.playerName,
